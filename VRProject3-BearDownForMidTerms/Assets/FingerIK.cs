...
public class FingerIK : MonoBehaviour
{
    ...
    [SerializeField] HandTrackingMap RH;
    [SerializeField] HandTrackingMap LH;
    ...
    private void SetProperties(TrackedFinger d1, Transform d2) {
        //d1.position = d2.position;
        d1.transform.rotation = d2.rotation;
        d1.transform.Rotate(d1.offset);
    }

    private void UpdateHand(HandTrackingMap h, OVRSkeleton s) {
        if (s == null || s.Bones.Count == 0) {
            return;
        }
        //SetProperties(h.Hand_WristRoot, s.Bones[(int)bid.Hand_WristRoot].Transform);
